...
<!-- Edit this file anytime to steer OpenSniper. It checks this at the start of each cycle. -->

## Trade Journal Management
Your journal file (`journal_OpenSniper.md`) must stay compact. Follow these rules:
...
## Focus Symbols
<!-- List symbols here to make OpenSniper prioritize them. Leave empty for normal watchlist. -->
<!-- Example: NVDA, TSLA, BTC -->

(none)

## Instructions
<!-- Add specific instructions for OpenSniper. Leave empty for normal operation. -->
<!-- Example: "Reduce position sizes to 50% — high volatility expected today" -->
<!-- Example: "Widen the OR window to 10 minutes for choppy opens" -->

(none)

## Risk Override
<!-- Override risk settings for OpenSniper. Leave empty for normal risk. -->
...
- **Price-source reconciliation:** `GET /api/positions` (platform price) is authoritative over any MCP-reported price when the two disagree by more than 0.1% — it's what actually triggers your stop-loss/take-profit fills. Note the divergence, don't act on the MCP number.
- **Position review before new entries, every cycle:** run the full Position Review Checklist on all open positions before scanning for new setups. This is ordering, not optional.
- **Non-Negotiable Exit Rules are hard rules, not overridable by directives here.** If a directive in this file conflicts with a fired exit rule (hard stop, profit target, time stop, momentum death, false-breakout reversal, profit lock), the hard rule still wins. Directives can adjust entry criteria, watchlist, and sizing — not disable exit discipline.
- **Decision Quality Framework:** Weighted confidence scoring instead of raw signal counting (with signal-family weighting — don't double-count correlated factors), data sanity checks, position-overlap checks via `GET /api/positions`, circuit breakers after losing streaks, and near-miss logging for calibration.
- **Pattern-learning sample-size floor:** don't adjust entry/exit thresholds based on fewer than ~15-20 comparable trades. A 3-loss streak triggers the circuit breaker but is not yet proof the strategy is broken.
...
- **Journal calibration:** Log every position review every cycle (numbers before narrative) — not just closes. Each closed-trade entry records a confidence score and whether the outcome matched that conviction level.
...
If you want to raise or lower the bar (e.g., "require score 8+/9 this week" or "suspend circuit breakers"), state it here. You cannot use this section to suspend the Non-Negotiable Exit Rules themselves.
